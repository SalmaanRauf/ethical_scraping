...
class BingDataExtractionAgent:
    ...
    def _run_once(self, agents_client, agent, user_prompt: str) -> Tuple[str, List[Dict[str, str]], List[str]]:
        ...
        logger.info(f"Starting GWBS search: {user_prompt[:100]}...")
        run = agents_client.runs.create_and_process(thread_id=thread.id, agent_id=agent.id)
        
        if run.status == "failed":
            logger.error(f"GWBS run failed: {run.last_error}")
            raise RuntimeError(f"Run failed: {run.last_error}")

        msg = agents_client.messages.get_last_message_by_role(thread_id=thread.id, role=MessageRole.AGENT)
        if not msg:
            logger.error("No agent response received")
        ...
        if queries:
            logger.debug(f"Search queries used: {queries}")
            
        logger.info(f"GWBS search completed - {len(citations)} citations found")
    ...
    def _run_agent_task(self, user_prompt: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting agent task for prompt: {user_prompt[:80]}...")
        ...
        with AIProjectClient(endpoint=self.project_endpoint, credential=self.credential) as project_client:
            ...
            try:
                ...
                if not cites:
                    logger.warning("No citations found in first pass, attempting corrective search")
                    follow_up = (
                        user_prompt
                        + "\n\nFOLLOW-UP:\n"
                    ...
                    if cites2:
                        ...
                        logger.info(f"Corrective search successful - found {len(cites2)} citations")
                    else:
                        logger.warning("Corrective search also returned no citations")
                    queries.extend(queries2 or [])

                bullets = self._citations_to_markdown(cites)
                logger.info(f"Task completed - summary length: {len(body)}, citations: {len(cites)}")

            finally:
                # Always clean up the ephemeral agent
                try:
                    agents_client.delete_agent(agent.id)
                    logger.debug("Ephemeral agent cleaned up")
                except Exception as e:
                    logger.warning(f"Failed to clean up agent: {e}")
