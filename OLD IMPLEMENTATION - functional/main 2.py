...
async def handle_new_analysis(
    ...
    """Handle new company analysis with enhanced progress tracking."""
    ...
    await cl.Message(
        content=f"""🔎 **Starting Comprehensive Analysis of {company}**
I'll search across 5 key areas:
• 📄 SEC Filings & Regulatory disclosures
• 📰 Recent news & market developments  
• 🏛️ Government procurement & contracts
• 💵 Earnings calls & financial guidance
• 🌐 Industry context & competitive landscape
This typically takes 2-3 minutes as I gather and analyze information from multiple sources."""
    ).send()
    try:
        if os.getenv("ENABLE_TOOL_ORCHESTRATOR", "false").lower() in ("1", "true", "yes"):
            cref = CompanyRef(name=company, ticker=ticker)
            # Track progress for user display
            progress_messages = []
            total_citations = 0
            async def _enhanced_progress(action: str, data: Dict):
                """Enhanced progress callback with rich user feedback."""
                nonlocal progress_messages, total_citations
                try:
                    if action == "start":
                        scope_name = data["scope"].replace('_', ' ').title()
                        current = data["current"]
                        total = data["total"]
                        message = await cl.Message(
                            content=f"**{current}/{total}** 🔍 **Searching {scope_name}** for {company}..."
                        ).send()
                        ...
                        current = data["current"]
                        total = data["total"]
                        # Update the last progress message with results
                        if progress_messages:
                            try:
                                await progress_messages[-1].update(
                                    content=f"**{current}/{total}** ✅ **{scope_name} completed** - Found **{citations} citations**"
                                )
                            except Exception:
                        ...
                        total_citations = data["total_citations"]
                        completed = data["completed_scopes"]
                        total = data["total_scopes"]
                        await cl.Message(
                            content=f"""📊 **Research Complete!**
I've reviewed **{total_citations} citations** across {completed} data sources. Now synthesizing the key insights..."""
                        ).send()
                    elif action == "analyzing":
                        await cl.Message(
                            content="🧠 **Analyzing findings** - Identifying significant events and business implications..."
                        ).send()
                except Exception as e:
                    logger.warning(f"Enhanced progress callback failed: {e}")

            briefing = await ors.full_company_analysis(
                cref,
                bing_agent=bing_agent,
                analyst_agent=analyst_agent,
                progress=_enhanced_progress
            )
            # Send final citation impact message
            citation_impact = "high" if total_citations > 30 else "moderate" if total_citations > 15 else "limited"
            await cl.Message(
                content=f"""✅ **Analysis Complete**
I identified **{len(briefing.events)} significant events** for {company} after reviewing **{total_citations} sources**. This represents **{citation_impact} information coverage**."""
            ).send()
            # Save context and present results (your existing code)
            blob = AnalysisBlob(
            ...
            if extra_topics:
                try:
                    ...
                    answer, citations = await ors.follow_up_research(
                        cref,
                        original_text or "",
                        bing_agent=bing_agent,
                        analyst_agent=analyst_agent,
                        ctx_blob=blob.to_dict()
                    )
                    cite_lines = []
            ...
            gwbs_lines = [f"# 🔍 Raw Research Results for {company}"]
            gwbs_lines.append("## Grounding with Bing Search (GWBS) Findings")
            gwbs_lines.append("")
            for section, section_data in gwbs.items():
                if not isinstance(section_data, dict):
                    continue
                gwbs_lines.append(f"### {section.replace('_', ' ').title()}")
                gwbs_lines.append("")
                summary = section_data.get("summary", "")
                if summary:
                    gwbs_lines.append(summary)
                    gwbs_lines.append("")
                citations_md = section_data.get("citations_md", "")
                if citations_md:
                    gwbs_lines.append("**Sources:**")
                    gwbs_lines.append(citations_md)
                    gwbs_lines.append("")
            await cl.Message("\n".join(gwbs_lines)).send()
    ...
    try:
        if os.getenv("ENABLE_TOOL_ORCHESTRATOR", "false").lower() in ("1", "true", "yes"):
            ...
            async with lock:
                ...
                await cl.Message("🔄 Processing your follow-up question...").send()
                ...
                # Define progress callback for user-facing updates
                async def _progress_update(message: str):
                    """Send progress updates to the user."""
                    try:
                        # Use a different style to distinguish from final answers
                        await cl.Message(f"**Progress**: {message}").send()
                    except Exception as e:
                ...
                async def _run_followup():
                    return await ors.follow_up_research(
                        ...
                        ctx_blob=blob_dict,
                        progress=_progress_update  # Pass the progress callback
                    )
                ...
                # IMPROVED VALIDATION: Check for empty or invalid responses
                if not answer or not isinstance(answer, str) or answer.strip() == "":
                    logger.warning(f"Empty answer returned for follow-up: {user_text}")
                    answer = "I conducted the search but couldn't find specific information about that topic. The search may not have returned relevant results for your question."
                if citations is None:
                    citations = []
                # Log what we found for debugging
                logger.info(f"Follow-up result - answer length: {len(answer)}, citations: {len(citations)}")
                cite_lines = []
                for c in citations or []:
                    try:
                        title = getattr(c, 'title', None) or getattr(c, 'url', 'Unknown')
                        url = getattr(c, 'url', '#')
                ...
                if not answer.strip():
                    answer = "I couldn't generate a response from the search results. Please try rephrasing your question."
                await cl.Message(answer + footer).send()
        else:
            # Legacy follow-up handler
            result = fup.handle_follow_up(ctx, user_text)
            if not isinstance(result, dict) or "answer" not in result:
                logger.error("Invalid response from legacy follow-up handler")
                await cl.Message("I couldn't process your question. Please try rephrasing it.").send()
                return
            answer = result.get("answer", "")
            cites = result.get("citations", [])
            # Validate legacy response too
